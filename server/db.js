...
export class RegisterModel {
    ...
    try {
      ...
      if (vipUsers.length > 0) {
        return {
          status: false,
          error:
            "Ya tienes comprano una entrada con este correo electrónico intenta con otro.",
        };
      }
  ...
  static async save_order_oktoberfest({
    ...
    const connection = await mysql.createConnection(config);
    const numero_personas = items.reduce(
  ...
  static async pagar_gafete_vip(id) {
    ...
    try {
      const [result] = await connection.query(
        ...
        [id, "compra-en-sitio-proveedor", "compra-en-sitio-proveedor", "5800"]
      );
      return {
        status: true,
        insertId: result.insertId,
